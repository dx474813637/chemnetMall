				...
				<view class="header u-p-18" :style="{
					...
					<view class="title-bar u-p-20 u-flex u-flex-between u-flex-items-center u-font-30">
						...
						<view class="item u-flex-1 u-text-center">
							<text class="u-font-34 text-base">搜索CAS/名称</text>
						</view>
	...
	export default {
		...
		methods: {
			...
			async getData() { 
				...
				const res = await this.$api.chemnet_product_search({
					params: {
						terms: this.keyword,
						p: this.curP
					}
				...
				if(res.code == 1) {
					this.list = [...this.list, ...res.list.list_search]
